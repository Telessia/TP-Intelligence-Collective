...
to create-rooms-and-roads
  ...
  resize-world -1 (dim-room + dim-road + dim-room + 1) -1 (dim-room + 1)
  ask patches [
    if pcolor = black [
      if pxcor >= 0 AND pxcor <= dim-room AND pycor >= 0 AND pycor <= dim-room
        [ set pcolor white ]
      if pxcor >= dim-room + dim-road AND pxcor <= dim-room + dim-room + dim-road AND pycor >= 0 AND pycor <= dim-room
        [ set pcolor white ]
      if pxcor >= dim-room AND pxcor <= dim-room + dim-road  AND pycor > dim-room / 2 - 1 AND pycor < dim-room / 2 + 1
        [ set pcolor white ]
  ] ]
end
